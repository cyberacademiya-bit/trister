# 📋 Тристер — полное состояние проекта

> Этот файл фиксирует всё, что сделано. Открывай его, чтобы вспомнить, где остановились.
> Для нового чата с Клодом: скажи «прочитай PROJECT_STATE.md в папке Тристер».

Обновлено: 2026-07-15 (Бишкек)

---

## Что это
Автономный мульти-рыночный торговый **робот на демо-счетах** + дашборд + работа 24/7.
Папка: `/Users/aim/Проекты/Тристер` · GitHub: `github.com/cyberacademiya-bit/trister`

## 4 подключённых демо-счёта (всё игрушечные деньги)
| Счёт | Что торгует | Баланс | Ключи в .env |
|---|---|---|---|
| Binance Spot | крипта + золото (PAXG) | ~$10k | BINANCE_API_KEY/SECRET |
| Binance Futures | фьючерсы, ЛОНГ+ШОРТ+стоп | ~$5k | FUTURES_KEY/SECRET |
| Alpaca | акции США | $100k | ALPACA_KEY/SECRET |
| Локальный | все рынки (симулятор) | $10k | — |

**Реальные деньги не тронуты нигде.** Ключи лежат в `.env` (не в git) и в GitHub Secrets.

## Как запускать (в терминале)
```bash
cd "/Users/aim/Проекты/Тристер" && source .venv/bin/activate
python trader.py --universe      # крипта + золото
python futures_bot.py            # фьючерсы (лонг/шорт)
python alpaca_bot.py             # акции
python paper_trader.py           # локальный, все рынки
python scanner.py                # сигналы по всем рынкам
python multi_strategy.py         # турнир всех стратегий
python gather_data.py            # собрать данные дашборда
open dashboard.html              # открыть дашборд локально
```

## Стратегии — МУЛЬТИ-режим (разная стратегия на каждом счёте)
По просьбе Усона все стратегии работают вживую одновременно, каждая на своём счёте:
| Счёт | Стратегия | Файл |
|---|---|---|
| Binance Spot | **sma** | config.STRATEGY="sma" |
| Binance Futures | **trend_ls** (лонг/шорт) | futures_bot.STRATEGY |
| Локальный | **trend** (акции+сырьё) | paper_trader.STRATEGY="trend" |
| Alpaca | **sma** | config.STRATEGY="sma" |

Размер сделки: **$500** (config.POSITION_USD / futures NOTIONAL_USD). Демо.
Тест 99 гипотез + турнир: **sma_20_50** — самая устойчивая по риску. Но на демо гоняем все — это учёба (Усон: «пусть минус тоже будет уроком»). Вывод из тестов: простота + риск-менеджмент побеждают.

## Дашборд и хостинг
- Локально: `dashboard.html` (двойной клик).
- Онлайн: `site/index.html` тянет `site/dashboard_data.json`.
- **Vercel** — задеплоен (`trister.vercel.app`), но новые деплои ЗАБЛОКИРОВАНЫ (просит оплату). Root Directory=`site`, Framework=Other.
- **GitHub Pages** — ✅ **ВКЛЮЧЁН И РАБОТАЕТ** (бесплатно). Репо публичный, Pages source=GitHub Actions, `.github/workflows/pages.yml` пересобирает сайт при каждом push. Сайт: **https://cyberacademiya-bit.github.io/trister/** (обновляется сам после каждого часового прогона бота).

## Авто 24/7
GitHub Actions (`.github/workflows/bot.yml`) — два расписания:
- **Каждый час** (`0 * * * *`): живая торговля + **проверка стоп-лосса** («продать при риске»). Фьючерсный бот пишет каждый стоп/сделку в журнал `trades.csv`.
- **Раз в день** (`0 14 * * *` = 20:00 Бишкек): дополнительно тяжёлый турнир всех стратегий + локальный симулятор.

Мак может быть выключен. Для публичного репо минуты Actions **бесплатны** → почасовой режим ничего не стоит.
Вход в позицию — по дневным свечам (меньше дёрганья), а стоп-лосс проверяется каждый час по живой цене.

### ⚠️ ВАЖНО: Binance блокирует облако (ошибка 451)
Binance отдаёт **HTTP 451 «restricted location»** серверам GitHub (дата-центры США/облака в его бан-листе). Поэтому **на бесплатном облаке торгуют только:**
- ✅ **Alpaca** (акции США) — работает
- ✅ **Локальный симулятор** (крипта+золото+акции через yfinance) — работает, каждый час
- ❌ **Binance Spot + Futures** — гео-блок. Боты ловят 451 и выходят красиво (ℹ️, exit 0), прогон остаётся зелёным.

**Чтобы Binance-демо тоже торговал 24/7:** запускать с Мака (`python trader.py --loop`) ИЛИ арендовать сервер ~$5/мес в разрешённой стране. Крипта+золото и так торгуются на локальном симуляторе — те же рынки.

## Что осталось / идеи на потом
- [ ] Довести GitHub Pages (3 шага выше)
- [ ] По желанию: чаще обновлять (поменять cron), стоп-лоссы тоньше, время Alpaca на Бишкек
- [ ] Учёба: папка `Книги/` (21 книга) + `Фондовый_рынок_справочник.docx` + `План_обучения.md`

## Учебные материалы в папке
`Фондовый_рынок_справочник.docx` (~69 стр) · `План_обучения.md` (44 книги + 34 канала) · `Книги/` (21 скачанная книга).
